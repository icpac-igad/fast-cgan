...
from fastcgan.tools.constants import OPEN_IFS_DATA_VARIABLES
from fastcgan.tools.data_sync import run_ecmwf_ifs_sync, run_sftp_rsync
...
def syncronize_open_ifs_forecast_data(
    ...
    if not get_data_sycn_status():
        ...
        for grib2_file in open_ifs:
            post_process_ecmwf_grib2_dataset(grib2_file_name=grib2_file, force_process=True)
    ...
    for ifs_date in ifs_dates:
        if ifs_date not in gan_dates:
            ...
            try:
                ...
                cgan_file_path = get_data_store_path(source="jobs") / "cgan" / f"GAN_{data_date.strftime('%Y%m%d')}.nc"
                save_to_new_filesystem_structure(
